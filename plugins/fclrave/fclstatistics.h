...
public:
    ...
    void Display(const std::string& fileprefix="") {
        ...
        FOREACH(ittiming, timings) {
            ...
            FOREACH(ittimingvector, ittiming->second) {
                ts.emplace_back(std::chrono::duration_cast<std::chrono::nanoseconds>((*(ittimingvector->end()-1) - *(ittimingvector->begin()))));
            }
            auto sum = std::accumulate(std::begin(ts), std::end(ts), std::chrono::nanoseconds(0));

            ss << ittiming->first << ": ";
            ss << "sum: " << sum.count()  << std::endl;
            totaltime = totaltime + sum;
            RAVELOG_INFO(ss.str());
        }
        RAVELOG_INFO("Check Collision time in total: %ld", totaltime.count());
